...
def test_meta_graph_truncate_augmented_ss_bounded_subcomplex(seeded: int):
    """With only bounded cells, ``truncate`` appends ``1`` and adds no duplicate nodes."""
    set_seeds(seeded)
    import torch.nn as nn
    ...
    assert set(meta_plain.nodes()) == set(meta_tr.nodes())
    assert not any(isinstance(n, tuple) for n in meta_tr.nodes())
