...
:func:`~relucent.search.boundary_search.discover_boundary_complex` (MIP pricing +
...
:func:`~relucent.graph.incidence.collect_meta_face_edges` builds directed
``k-cell → (k−1)-face`` incidences by zeroing **every** index where
``ss_i ≠ 0`` (via :func:`~relucent.graph.incidence.ss_nonzero_indices`), then keeping
...
(``_apply_ambient_boundary_shis`` in :mod:`relucent.search.boundary_search`), then
runs the same ``set_contracted_shis`` pass during finalize.

Certification
-------------

Certification is orchestrated by :func:`~relucent.verify.certify.certify_complex`.
:meth:`~relucent.core.complex.Complex.bfs` with ``verify=True`` runs it automatically
after a **complete** search via ``finalize_ambient_search`` at
:class:`~relucent.verify.certify.CertifyLevel.COMPLETE`. Call
:meth:`~relucent.core.complex.Complex.certify` to re-run certification manually.

:class:`~relucent.verify.certify.CertifyLevel` is cumulative:

* ``COMBINATORIAL`` — flip-SHI symmetry
  (:func:`~relucent.graph.incidence.verify_flip_shi_symmetry`), dual-graph certification
  (:func:`~relucent.graph.incidence.certify_dual_graph`), and contracted-slice SHI checks
  (:func:`~relucent.graph.incidence.verify_contracted_shis`) when applicable.
...
:func:`~relucent.search.exploration.finalize_boundary_complex` also:

* Verifies ambient coface feasibility per cell
  (:func:`~relucent.verify.certify.verify_boundary_cell`)
* Builds the dual graph with ``require_complete=verify``
* Runs :func:`~relucent.verify.certify.verify_arrangement_genericity`
* Calls :func:`~relucent.graph.incidence.set_contracted_shis` before ``certify_complex``

Exploration flags
~~~~~~~~~~~~~~~~~

After search or certification, :class:`~relucent.core.complex.Complex` records:
...
:meth:`~relucent.core.complex.Complex.assert_topology_ready` (requires both flags).
Tests often use :func:`~relucent.search.exploration.explore_for_topology`.
...
3. **Boundedness** — 0-face incidence on 1-cells and ascending sweep
   (:func:`~relucent.graph.incidence.classify_one_cells_finite_from_face_edges`,
...
1. **MIP pricing** — :func:`~relucent.search.boundary_mip.price_boundary_witness` finds
   witnesses for new connected components on the slice ``ss[i] = 0``.
2. **Slice BFS** — :func:`~relucent.search.boundary_search.boundary_searcher` explores
   each component with ``ss[boundary_shi] = 0`` fixed and SHI subsets that
   exclude the boundary hyperplane.
3. **Finalize** — :func:`~relucent.search.exploration.finalize_boundary_complex`:
